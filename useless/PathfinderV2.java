...
public class Pathfinder {
	...
	private static Vector<PathNode> getPossibilities(PathNode currentNode, int direction) {
		...
		for(int i = 0; i < 3; ++i) {
			cell = getNeighbourCellFromDirection(directions[i]);
			
			if(DEBUG)
				System.out.println(directions[i] + " " + cell);
			
			if(cell != null && currentNode.checkCell(cell))
				possibilities.add(new PathNode(cell, directions[i]));
		}
	...
	private static int[] besideDirections(int direction) {
		...
		if(DEBUG)
			System.out.println("Beside directions : " + result[0] + " " + result[1] + " " + result[2]);
		return result;
	}
	...
	private static int[] otherDirections(int direction) {
		...
		if(DEBUG) {
			System.out.print("Other directions : ");
			for(int i = 0; i < 5; ++i)
