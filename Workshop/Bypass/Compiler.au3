	...
	Switch $msg
		...
		Case $Button2
			If GUICtrlRead($debug1)=$GUI_CHECKED Then
				Run(@ComSpec & ' /k "C:\Progra~2\AIRSDK_Compiler\bin\amxmlc -debug" ' & $path1)
			Else
				Run(@ComSpec & ' /k "C:\Progra~2\AIRSDK_Compiler\bin\amxmlc" ' & $path1)
			EndIf
		Case $Button3
		...
		Case $Button4
			If GUICtrlRead($debug2)=$GUI_CHECKED Then
				Run(@ComSpec & ' /k "C:\Progra~2\AIRSDK_Compiler\bin\fdb"')
				Sleep(1000)
				Send("run"&"{ENTER}")
				Run("C:\Windows\System32\cmd" & ' /k "C:\Progra~2\AIRSDK_Compiler\bin\adl" ' & $path2,"C:\Progra~2\AIRSDK_Compiler\bin",@SW_MINIMIZE)
				;Sleep(1000)
				;Send("continue"&"{ENTER}")
			Else
				Run(@ComSpec & ' /k  "C:\Progra~2\AIRSDK_Compiler\bin\adl"' & $path2)
			EndIf
	EndSwitch
WEnd